...
image: https://img.yumeko.site/file/blog/cover/1781780694206_VectorMatrixNorm.webp
status: published
---
...
![LpUnitBalls.png](https://img.yumeko.site/file/blog/articles/1781780744321_LpUnitBalls.webp)
### 3.3 几何直觉：单位球
...
> [!TIP] L1 和 $L^{\infty}$ 的对称性
> 矩阵的 L1 范数是**列和**最大值，$L^{\infty}$ 范数是**行和**最大值。这不难记——下标 1 对应竖着的列（像数字 1），下标 ^inf 对应横着的行。
...
| Chebyshev（$L^{\infty}$） |                        $\max_i \|x_i-y_i\|$                         |    最大分量差异敏感的场景     |     
...
| 向量 $L^{\infty}$ | 最大分量     | $\|x\|_\infty = \max \|x_i\|$    | 鲁棒性分析          |
