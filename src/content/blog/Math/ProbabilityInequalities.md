...
![图0: 概率不等式全景——从马尔可夫到霍夫丁](https://img.yumeko.site/file/blog/ProbabilityInequalities.png)

> **🖼️ AI 生图提示词：**
>
> ```
> 一张宽幅 Banner（宽高比 2.35:1），用于概率不等式完全指南的博客封面。
> 设计概念：从左侧的"弱假设/松上界"到右侧的"强假设/紧上界"的递进关系。
> 左侧放置马尔可夫不等式的几何示意（一个分布尾部被一条曲线压制），
> 中间是切比雪夫不等式的方差椭球示意，
> 右侧是切尔诺夫/霍夫丁的指数衰减曲线。
> 底部用箭头连接，标注"假设由弱到强，界由松到紧"。
...
|:--|:--|:--|:--|
| $E[X]$ | $1.000$ | $0.368$ | 2.7× |
| $3E[X]$ | $0.333$ | $0.050$ | 6.7× |
| $5E[X]$ | $0.200$ | $0.007$ | 29× |

马尔可夫界是**绝对安全的上界**，但远不是紧的。其普适性（仅需 $X \ge 0$ 和 $E[X]$ 存在）的代价是保守性。

![图1: 马尔可夫界 vs 精确概率——以指数分布为例](https://img.yumeko.site/file/blog/ProbabilityInequalities/MarkovBound.png)

> **🖼️ AI 生图提示词：**
>
> ```
> 一张简洁的科学对比图，展示马尔可夫不等式给出的上界与指数分布真实尾部概率的差距。
> 横轴为 a（从 0 到 5E[X]），纵轴为概率（0 到 1）。
> 两条曲线：红色虚线为马尔可夫上界 P(X≥a) ≤ E[X]/a（缓慢下降的双曲线），
> 蓝色实线为指数分布的真实尾部概率 e^{-λa}（呈指数级快速下降）。
> 在 a=E[X]、a=3E[X]、a=5E[X] 处用竖直虚线标注，并标注两者的数值差距。
> 图例清晰。白色背景，细坐标轴线，学术图表风格。
> ```
...
![图2: 切比雪夫不等式的推导——马尔可夫 + 平方变换](https://img.yumeko.site/file/blog/ProbabilityInequalities/ChebyshevDerivation.png)

> **🖼️ AI 生图提示词：**
>
> ```
> 一张教学示意图，展示切比雪夫不等式从马尔可夫不等式的推导过程。
> 分为三步，从左到右排列：
> 第一步"平方变换"：将随机变量 Y 变换为 X=(Y-μ)²，图示一个正态分布被平方后变成卡方形状。
> 第二步"应用马尔可夫"：对非负 X 应用马尔可夫不等式 P(X≥a) ≤ E[X]/a，取 a=ε²。
> 第三步"得到切比雪夫"：P(|Y-μ|≥ε) ≤ σ²/ε²。
> 每步用矩形框标出，箭头连接。框内用 LaTeX 风格数学符号标注。
> 简洁矢量图风格，白色背景，柔和的蓝白配色。数学教科书插图。
> ```
...
![图3: 切尔诺夫界的三步推导——指数化、马尔可夫、参数优化](https://img.yumeko.site/file/blog/ProbabilityInequalities/ChernoffBound.png)

> **🖼️ AI 生图提示词：**
>
> ```
> 一张教学示意图，展示切尔诺夫界的三步推导策略。
> 第一步"指数化"：将随机变量 X 变换为 e^{tX}（图示一条指数曲线，将 X 的取值范围映射到正半轴）。
> 第二步"马尔可夫"：对非负的 e^{tX} 应用马尔可夫不等式，得到 P(X≥a) ≤ e^{-ta}·E[e^{tX}]。
> 第三步"优化 t"：通过求导找到最优的 t 值使上界最紧（图示一条 U 形曲线，最低点为最优 t）。
> 每步用矩形框标出，箭头连接。右下角小图展示切尔诺夫界（指数衰减）与切比雪夫界（多项式衰减）的对比。
> 简洁矢量图风格，白色背景，柔和的蓝色和橙色配色方案。
> ```
...
**（1）方差非负**：$f(x) = x^2$ 是凸函数 → $E[X]^2 \le E[X^2]$，即 $\operatorname{Var}(X) = E[X^2] - E[X]^2 \ge 0$。

**（2）KL 散度非负**：$f(x) = -\ln x$ 是凸函数 → $D_{\text{KL}}(P \| Q) = E_P[-\ln(q/p)] \ge -\ln(E_P[q/p]) = -\ln(1) = 0$。

**（3）EM 算法的收敛性**：EM 算法的 E 步构造了一个凹函数的下界，M 步最大化该下界——琴生不等式保证了每次迭代后似然函数不降。

**（4）算术平均 ≥ 几何平均**：$f(x) = -\ln x$ 凸 → $-\ln E[X] \le E[-\ln X]$ → $E[X] \ge \exp(E[\ln X])$。
...
![图4: 六大不等式层级对比——假设从弱到强，界从松到紧](https://img.yumeko.site/file/blog/ProbabilityInequalities/InequalityComparison.png)

> **🖼️ AI 生图提示词：**
>
> ```
> 一张综合对比图，展示六条概率不等式的层级关系。
> 中央大图为坐标系，横轴为"所需假设强度"（从弱到强），纵轴为"尾部衰减速度"（从 O(1/n) 到 O(e^{-n})）。
> 六条不等式从左下到右上依次排列：
> 马尔可夫（最左下：假设最弱，衰减 O(1/a)）、
> 柯西-施瓦茨（中间偏左）、
> 切比雪夫（中间，衰减 O(1/n)）、
> 琴生（独立位置，标注"不直接给出概率界"）、
> 切尔诺夫（右上，衰减 O(e^{-n})，标注"需要 MGF"）、
> 霍夫丁（最右上，衰减 O(e^{-n})，标注"需要有界+独立"）。
> 每条不等式用不同颜色的圆点标出，大小表示应用广泛程度。
> 从左上到右下的虚线箭头标注"推导关系"：
> 马尔可夫→切比雪夫（平方化），马尔可夫→切尔诺夫（指数化），切尔诺夫→霍夫丁（有界条件下应用）。
> 白色背景，专业学术图表风格，中英文混合标签。
> ```
