...
>
> ```
> 一张简洁的科学对比图，展示 Runge 函数 f(x)=1/(1+25x²) 的插值结果。左图：真实的 Runge 函数（蓝色光滑钟形曲线）。中图：5 个等距节点的多项式插值，边缘出现轻微振荡。右图：15 个等距节点的多项式插值，两端出现剧烈振荡（红色发散曲线）。x 轴范围 -1 到 1，三个子图共用一个 y 轴刻度（0 到 1.2）。数据点以黑色圆点标出。专业学术图表风格，白色背景，细坐标轴线，每个子图含图例。数学教科书插图风格。
> ```
...
>
> ```
> 一张三栏对比图，展示同一组 6 个散点数据在不同分段插值方法下的结果。上图："分段线性"——直线段连接各点，每个数据点处可见明显尖角。中图："分段二次"——抛物线段连接，较为光滑但仍可见折痕。下图："三次样条"——完全光滑的连续曲线穿过所有点，无任何可见折痕。所有子图中数据点以黑色实心圆标出，各子图清晰标注方法名称。简洁矢量图风格，白色背景，数学教科书插图。
> ```
...
>
> ```
> 一张教学示意图，说明三次样条各段多项式的构造方式。展示一个坐标平面，包含 5 个数据点（P0 到 P4），以黑色圆点标出。x 轴标注：x₀, x₁, x₂, x₃, x₄。每对相邻点之间用不同颜色的三次曲线段连接。图下方展示结构：对区间 [xᵢ, xᵢ₊₁]，标注四个关键参数——步长 hᵢ = xᵢ₊₁ - xᵢ、端点二阶导数 Mᵢ 和 Mᵢ₊₁、以及函数值 yᵢ 和 yᵢ₊₁。附小图展示 Sᵢ(x) 公式，高亮 Mᵢ 相关项。简洁矢量图，数学教科书风格，白色背景。
> ```
...
>
> ```
> 一张三栏科学对比图，展示同一组 6 个数据点在三种不同边界条件下的三次样条插值结果。所有子图共享相同的数据点、坐标轴和刻度。上图："自然样条"——曲线在两端以零曲率趋近（自然释放，两端趋于直线）。中图："固定边界样条"——曲线在两端按指定切线方向趋近（用小箭头标示端点处预设的斜率）。下图："Not-a-Knot 样条"——前两段和后两段各自合并为单条三次曲线（边界附近过渡更平滑）。用浅色叠加高亮三个方法差异最大的边界区域（x₀-x₁ 和 xₙ₋₁-xₙ）。数据点为黑色圆点，曲线为蓝色。简洁学术图表风格，白色背景，细坐标轴线。
> ```
...
>
> ```
> 三次样条系统三对角矩阵方程 Ax = d 的可视化表示。将 (n+1)×(n+1) 矩阵 A 以网格形式展示，仅三条对角线带着色（主对角线蓝色，次对角线和超对角线浅蓝色），其余位置为空/白色（显示为 0）。未知向量 x = [M₀, M₁, ..., Mₙ]ᵀ 以列向量形式显示在矩阵右侧。右端向量 d = [0, d₁, d₂, ..., dₙ₋₁, 0]ᵀ 显示在另一列。用不同颜色（绿色）高亮第一行和最后一行（边界条件：M₀=0, Mₙ=0）。矩阵元素显示符号值：1, μ₁, 2, λ₁, μ₂, 2, λ₂ 等。简洁矢量图，数学符号风格，标签使用 LaTeX 字体。白色背景。
> ```
...
>
> ```
> 一张流程图风格的示意图，解释追赶法（Thomas 算法）分两步求解三对角方程组。分为左右两个面板。左面板："前向消元（追）"——箭头自上而下流经数组索引 1,2,...,n，展示消元公式 cᵢ' = cᵢ/(bᵢ - aᵢ·cᵢ₋₁') 和 dᵢ' = (dᵢ - aᵢ·dᵢ₋₁')/(bᵢ - aᵢ·cᵢ₋₁')。显示修改后的系数依次计算。右面板："后向回代（赶）"——箭头自下而上流经数组索引 n, n-1,...,0，展示回代公式 Mᵢ = dᵢ' - cᵢ'·Mᵢ₊₁。时间复杂度标注：每步 "O(n)"，底部标注 "总计：O(n)"。简洁算法图风格，中文标注"追"和"赶"，柔和的蓝色和橙色配色方案。白色背景，专业外观。两面板之间用箭头连接表示数据流向。
> ```
...
>
> ```
> 一张简洁的数学图表，展示恰好 4 个数据点的自然三次样条插值：P0(0,0), P1(1,2), P2(2,1), P3(3,3)。四个数据点以大号黑色实心圆标出，附坐标标注。样条曲线为光滑的蓝色曲线，穿过所有数据点。三段边界（x=1 和 x=2）以细虚线标出。每段下方标注："[0,1]: S₀(x) = 3x - x³"、"[1,2]: S₁(x)"、"[2,3]: S₂(x)"。y 轴显示 0,1,2,3。网格线极淡。同时用灰色虚直线连接相同数据点（分段线性）作为对比，展示三次样条更加光滑。白色背景，教科书图表风格，细边框，坐标轴标签清晰可读。
> ```
...
>
> ```
> 一张 3×2 网格的综合对比图，展示同一组 8 个散点数据（非均匀分布，部分聚集）在 6 种不同插值方法下的结果。所有子图共用相同的数据点（黑色圆点）、相同的 x 轴和 y 轴（x: 0 到 10, y: 0 到 8）以及相同的样式设置。网格布局：
> 左上："拉格朗日插值（7 次）"——单条全局多项式，边缘出现 Runge 振荡。
> 中上："分段线性"——直线段连接，尖角明显。
> 右上："三次样条（自然）"——光滑曲线，本文主题，用加粗线条或浅色光晕高亮。
> 左下："Akima 样条"——与三次样条类似，但在快速变化处超调更少。
> 中下："B 样条"——光滑但可能不精确经过所有点。
> 右下："RBF 插值"——光滑，经过所有点但可能振荡。
> 三次样条子图（右上）用浅色边框或星标强调，表示为推荐方法。简洁学术风格，白色背景，每个子图独立标注。适合博客或教科书对比使用。
> ```
