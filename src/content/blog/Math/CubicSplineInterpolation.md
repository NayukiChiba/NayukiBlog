...
![图0: 三次样条插值概览——从离散数据点到光滑曲线](https://img.yumeko.site/file/blog/CubicSpline/CubicSplineBanner.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A wide banner image (2.35:1 aspect ratio) for a blog post about cubic spline interpolation. Left third: scattered data points (black dots) on a faint grid background, looking discrete and disconnected. Center third: a smooth glowing blue curve gracefully flowing through all the data points, with subtle mathematical symbols (∫, f'', Mᵢ) floating around the curve in a semi-transparent aesthetic overlay. Right third: the final smooth spline curve extending elegantly. The composition should convey the transition from discrete points to smooth continuous curves. Dark to light gradient background, modern clean aesthetic, tech-blog style with a mathematical/engineering feel. Title space left intentionally empty for text overlay. Color palette: deep blue (#1a365d) to light blue (#ebf4ff), with white accents.
> ```
...
![图1: Runge现象——高次多项式插值的端点振荡](https://img.yumeko.site/file/blog/CubicSpline/RungePhenomenon.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A clean scientific chart comparing Runge function f(x)=1/(1+25x²) interpolation results. Left panel: the true Runge function (smooth bell curve in blue). Middle panel: 5-point polynomial interpolation showing slight edge wiggles. Right panel: 15-point polynomial interpolation showing violent oscillations at both ends (red diverging curves). The x-axis ranges from -1 to 1. All panels share the same y-axis scale (0 to 1.2). Data points marked as black dots. Professional academic figure style, white background, thin axis lines, legend in each panel. Mathematical textbook illustration style.
> ```
...
![图2: 分段插值对比——线性、二次与三次样条](https://img.yumeko.site/file/blog/CubicSpline/PiecewiseComparison.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A three-panel comparison figure showing different piecewise interpolation methods through the same 6 scattered data points. Top panel: "Piecewise Linear" — straight line segments connecting points, sharp corners visible at each data point. Middle panel: "Piecewise Quadratic" — parabolic segments with smoother but still visibly kinked connections. Bottom panel: "Cubic Spline" — perfectly smooth continuous curve flowing through all points with no visible kinks. Data points shown as black filled circles in all panels. Each panel labeled clearly. Clean vector graphic style, white background, mathematical illustration for a textbook.
> ```

![图3: 物理样条——弹性木条与压铁](https://img.yumeko.site/file/blog/CubicSpline/PhysicalSpline.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A technical illustration showing a draftsman's physical spline tool: a thin flexible wooden strip (light wood color, about 40cm long) bent smoothly and held in place by several metal weights (called "ducks" or "spline weights") on a drafting table with grid paper. The wooden strip forms a smooth curve passing through the weight positions. Each weight has a hook or point that holds the strip at specific coordinates. Pencil and ruler visible nearby. The curve formed naturally minimizes bending energy. Semi-realistic technical drawing style, warm lighting, top-down or slight angle view. Cross-section diagram inset showing how the weight holds the strip.
> ```
...
![图4: 三次样条分段构造示意——$M_i$ 与各段多项式的几何关系](https://img.yumeko.site/file/blog/CubicSpline/PiecewiseConstruction.png)

> **🖼️ AI 生图提示词：**
>
> ```
> An educational diagram illustrating how cubic spline segments are constructed piece by piece. Show a coordinate plane with 5 data points (P0 through P4) marked as black dots. The x-axis labels: x₀, x₁, x₂, x₃, x₄. Between each pair of consecutive points, draw a differently colored cubic curve segment. Under the plot, show the structure: for interval [xᵢ, xᵢ₊₁], label the four key parameters — step size hᵢ = xᵢ₊₁ - xᵢ, second derivatives Mᵢ and Mᵢ₊₁ at endpoints, and the function values yᵢ and yᵢ₊₁. Include small inset equations showing Sᵢ(x) formula with the Mᵢ terms highlighted. Clean vector diagram, math textbook style, white background.
> ```
...
![图5: 三种边界条件对比——同一组数据点下的不同样条行为](https://img.yumeko.site/file/blog/CubicSpline/BoundaryConditions.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A three-panel scientific comparison figure showing the same 6 data points interpolated with three different cubic spline boundary conditions. All panels share identical data points, axes, and scales. Top: "Natural Spline" — the curve approaches the endpoints with zero curvature (flattening out, like a released spring). Middle: "Clamped Spline" — the curve approaches endpoints with specified tangent directions (show small arrows indicating the prescribed slopes at endpoints). Bottom: "Not-a-Knot" — the first two and last two segments are each a single cubic (smoother transition near boundaries). Highlight the boundary regions (x₀-x₁ and xₙ₋₁-xₙ) with subtle colored overlays where the three methods differ most. Data points as black dots, curve in blue. Clean academic figure style, white background, thin axis lines.
> ```
...
![图8: 三对角矩阵结构——未知数 $M_i$ 的带状方程组](https://img.yumeko.site/file/blog/CubicSpline/TridiagonalMatrix.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A visual representation of a tridiagonal matrix equation Ax = d for the cubic spline system. Show the (n+1)×(n+1) matrix A as a grid where only three diagonal bands are colored (main diagonal in blue, sub-diagonal and super-diagonal in light blue), all other entries are empty/white (shown as 0). The unknown vector x = [M₀, M₁, ..., Mₙ]ᵀ shown as a column on the right of the matrix. The right-hand side vector d = [0, d₁, d₂, ..., dₙ₋₁, 0]ᵀ shown as another column. Highlight the first and last rows (boundary conditions: M₀=0, Mₙ=0) in a different color (green). The matrix entries should show symbolic values: 1, μ₁, 2, λ₁, μ₂, 2, λ₂, etc. Clean vector diagram, mathematical notation style, labels in LaTeX font. White background.
> ```
...
![图9: 追赶法（Thomas Algorithm）流程——两步 $O(n)$ 求解](https://img.yumeko.site/file/blog/CubicSpline/ThomasAlgorithm.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A flowchart-style diagram explaining the Thomas Algorithm for solving tridiagonal systems in two passes. Split into two side-by-side panels. Left panel: "Forward Sweep (追)" — arrows flowing top to bottom through array indices 1,2,...,n, showing the elimination formulas cᵢ' = cᵢ/(bᵢ - aᵢ·cᵢ₋₁') and dᵢ' = (dᵢ - aᵢ·dᵢ₋₁')/(bᵢ - aᵢ·cᵢ₋₁'). Show the modified coefficients being computed sequentially. Right panel: "Backward Sweep (赶)" — arrows flowing bottom to top through array indices n, n-1,...,0, showing the back-substitution formula Mᵢ = dᵢ' - cᵢ'·Mᵢ₊₁. Time complexity labels: "O(n)" for each pass, "Total: O(n)" at bottom. Clean algorithmic diagram style, Chinese labels for 追 and 赶, soft blue and orange color scheme. White background, professional appearance. Arrow connections between the two panels showing data flow.
> ```
...
![图6: 数值示例——四个数据点的自然三次样条曲线](https://img.yumeko.site/file/blog/CubicSpline/NumericalExample.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A clean mathematical plot showing a natural cubic spline interpolation through exactly 4 data points: P0(0,0), P1(1,2), P2(2,1), P3(3,3). The four data points are marked as large black filled circles with coordinate labels. The spline curve is drawn as a smooth blue curve passing through all points. The three segment boundaries (x=1 and x=2) are marked with subtle dashed vertical lines. Under each segment region, label: "[0,1]: S₀(x) = 3x - x³", "[1,2]: S₁(x)", "[2,3]: S₂(x)". The y-axis shows values 0,1,2,3. Grid lines are faint. Also plot the same points connected by dashed gray straight lines (piecewise linear) for comparison, showing how the cubic spline is smoother. White background, textbook figure style, thin borders, readable axis labels.
> ```
...
![图7: 六种插值方法对比——同一组数据，六种结果](https://img.yumeko.site/file/blog/CubicSpline/InterpolationComparison.png)

> **🖼️ AI 生图提示词：**
>
> ```
> A comprehensive 3×2 grid comparison figure showing 6 different interpolation methods applied to the same 8 scattered data points (non-uniformly spaced, some clustered). All panels share identical data points (black dots), identical x and y axes (x: 0 to 10, y: 0 to 8), and identical appearance settings. Grid layout:
> Top-left: "Lagrange (7th degree)" — one global polynomial showing Runge oscillation at edges.
> Top-middle: "Piecewise Linear" — straight segments, sharp corners.
> Top-right: "Cubic Spline (Natural)" — smooth curve, our main subject, highlighted with a subtle glow or thicker line.
> Bottom-left: "Akima Spline" — similar to cubic but less overshoot near rapid changes.
> Bottom-middle: "B-Spline" — smooth but may not pass exactly through all points.
> Bottom-right: "RBF Interpolation" — smooth, goes through points but may oscillate.
> The cubic spline panel (top-right) has a subtle colored border or star marker to emphasize it as the recommended method. Clean academic style, white background, each panel independently labeled. Ideal for a blog post or textbook comparison.
> ```
