...
tags:
  - Scikit-learn
description: SVR支持向量回归的数学原理、epsilon-管道损失与完整工程实现。
...
1. 理解 SVR 的 $\varepsilon$-不敏感损失函数——管道内误差不计，管道外线性惩罚。
...
| 名称 | 类型 | 作用 |
|---|---|---|
| $\varepsilon$-不敏感损失 | 损失函数 | $L_\epsilon(y, f(\mathbf{x})) = \max(0, \|y - f(\mathbf{x})\| - \epsilon)$——管道内误差为零 |
...
- $\varepsilon$ 越大管道越宽，更多样本落入管道内——模型更平滑，支持向量更少。
...
- 样本在管道内 $\rightarrow$ $\xi_i = 0$ 且 $\xi_i^* = 0$ $\rightarrow$ 不计损失，不是支持向量。
...
- $\gamma$ 控制每个样本的影响半径：$\gamma$ 大 $\rightarrow$ 影响局部 $\rightarrow$ 可能过拟合；$\gamma$ 小 $\rightarrow$ 影响全局 $\rightarrow$ 可能欠拟合。
...
| 数学维度 | 线性回归 | 正则化回归 | SVR |
|---|---|---|---|
| 损失函数 | 平方损失 $\|y - \hat{y}\|^2$ | 平方损失 + $\lambda R(\mathbf{w})$ | **$\varepsilon$-不敏感损失——管道内不计** |
...
- SVR 的数学核心是三层结构：$\varepsilon$-管道损失 + 平坦性正则化 + 核技巧非线性映射。
- $\varepsilon$-管道使管道内样本不计损失——它们不是支持向量，不参与预测，实现样本层面的稀疏性。
...
| 名称 | 类型 | 作用 |
|---|---|---|
| `loadSvrDataset()` | 方法 | 调用 `make_friedman1` 生成 200 样本 $\times$ 10 特征的非线性回归数据 |
...
- 与线性回归的合成数据（3 特征 x 纯线性公式）形成鲜明对比——SVR 的数据是故意非线性的。
...
- 200 样本 x 10 特征——与线性回归的数据规模相同（200 x 3），但非线性结构完全不同。
...
- SVR 数据来自 `make_friedman1`——200 样本 x 10 特征，目标函数高度非线性（乘积 + 平方 + 正弦）。
...
1. 理解 SVR 的 epsilon-管道直觉——为什么管道内的样本不参与决策。
...
| 名称 | 类型 | 作用 |
|---|---|---|
| epsilon-管道直觉 | 概念 | 管道宽度 $2\epsilon$——管道内的样本被"忽视"，管道外才受惩罚 |
...
- epsilon 是一种"宽容度"——告诉模型："误差在 +/-0.1 以内就不算错"。
- 这与 OLS 形成鲜明对比——OLS 对所有误差（无论多小）都给予平方惩罚，SVR 对小误差零容忍零惩罚。
- epsilon 越大 -> 管道越宽 -> 更多样本被"放过" -> 支持向量越少 -> 模型越简单。
...
- C 和 epsilon 不是独立的——调大 C 会让模型更努力覆盖管道外的点，可能需要调小 epsilon 来获得更细致的拟合。
- 先定 epsilon（根据目标变量的量级和能容忍的误差），再调 C（根据过拟合/欠拟合状态）。
- 支持向量数量是调参最直接的反馈——支持向量突然增多往往意味着 C 过大或 epsilon 过小。
...
| 复杂度控制 | 固定（特征数） | max_depth 硬约束 | **C + epsilon + gamma 软约束** |
| "看见"什么 | 全部样本平等 | 按分裂规则分区 | **只看支持向量——管道内样本被忽略** |
| 模型结构 | $\mathbf{w}$ 向量 | 二叉树 | **支持向量集 + RBF 核** |

## 常见坑

1. 以为 SVR 的"管道"会随训练数据自动选择最优宽度——epsilon 是超参数，需人工设定。
2. 忽略 $C$ 是正则化倒数——$C$ 越大 = 正则化越弱 = 越容易过拟合，直觉与其他模型的 alpha 相反。
3. 将 RBF 核的"无限维映射"理解成"一定能完美拟合"——无限维是理论能力，实际受 C、epsilon、gamma 和样本量限制。
4. 认为支持向量越少越好——过少的支持向量可能意味着欠拟合（管道太宽）。

## 小结

- SVR 的核心直觉是三层：epsilon-管道提供宽容度，平坦性正则化提供光滑性，RBF 核提供非线性。
- epsilon-管道使大量样本被"忽略"——这是 SVR 稀疏性的根源，也是它与所有"所有样本都参与"的回归方法最根本的区别。
- RBF 核将每个支持向量变成一个"影响中心"——最终拟合是多个高斯钟形曲线的加权叠加。
- 支持向量数量是模型复杂度的直接反映——比 R^2 或 MSE 更直观地告诉你模型"有多复杂"。
...
| `SVR(C=10.0, epsilon=0.1, kernel='rbf', gamma='scale')` | 类 | scikit-learn 提供的 epsilon-支持向量回归器 |
...
| 参数名 | 类型 | 说明 | 示例取值 |
|---|---|---|---|
| `epsilon` | `float` | epsilon-不敏感管道半宽——误差在 +/-epsilon 内不计损失 | `0.1` |
| epsilon 增大 | — | 管道更宽 -> 更多样本被容忍 -> 支持向量减少 -> 模型更简单 | — |
| epsilon 减小 | — | 管道更窄 -> 更少样本被容忍 -> 支持向量增多 -> 模型更复杂 | — |

### 理解重点

- `epsilon=0.1` 的取值需要结合目标变量的量级——Friedman1 的 $y$ 通常在 $[0, 25]$ 范围内，epsilon=0.1 是相对较小的管道。
- epsilon 和 $C$ 有交互效应——epsilon 决定"什么是误差"，$C$ 决定"误差有多大代价"。
- 与 OLS 的零容忍形成对比——OLS 相当于 epsilon=0 且用平方惩罚所有偏离。
...
- 线性核 SVR 等价于带 epsilon-管道损失的线性回归——失去了非线性能力，但保留了可解释性（有 `coef_`）。
...
- gamma 是 RBF 核最敏感的超参数——小变化可能导致从欠拟合到过拟合的剧烈转变。
- gamma 与 $C$ 有交互效应——gamma 大（局部性强）+ $C$ 大（强拟合）极易导致过拟合。
...
| 超参数数 | 0 | 1~2 | **4（C, epsilon, kernel, gamma）** |
...
- $C$（惩罚强度）、epsilon（管道宽度）、kernel（映射方式）、gamma（核宽度）构成 SVR 的超参数体系——四者联合决定了模型行为。
...
loadSvrDataset()
  ...
  - 5 model = trainSvrRegressionModel(X_train_s, y_train)
  - 6 y_pred = model.predict(X_test_s)
  - 7 plot_residuals(y_test, y_pred)
  - 8 plot_learning_curve(SVR(C=10.0, epsilon=0.1, kernel='rbf', gamma='scale'), X_train_s, y_train, scoring='r2')
```
...
- SVR 流水线为 8 步——与线性回归（6 步）相比多了标准化（34），与正则化回归（8 步）步骤数相同但无多模型循环。
- 标准化必须在切分之后——与正则化回归一致。
...
| `X_train_s` | `ndarray(160, 10)` | 标准化后——每列 mu=0, sigma=1 | — |
...
- 训练完成后，大部分样本的 $\alpha_i - \alpha_i^* = 0$——它们被 epsilon-管道"忽视"，不参与预测。
...
| `plot_learning_curve(...)` | 函数 | 生成训练/验证 R^2 曲线——诊断泛化趋势 |
...
- 右图（残差 vs 预测）重点观察残差是否有系统性弯曲——若残差呈现 U 形或正弦形模式，说明 RBF 核的 gamma 或 epsilon 设置不当。
...
- 如果训练 R^2 远高于验证 R^2——过拟合信号，考虑增大 epsilon 或减小 C。
- 如果训练和验证 R^2 都低——欠拟合信号，考虑减小 epsilon 或增大 C，或检查 gamma 设置。
...
| nSV 偏低（< 30%） | — | epsilon 管道宽——模型简单，可能欠拟合 |
| nSV 适中（30%~60%） | — | 模型复杂度合理——多数样本被管道容忍 |
| nSV 偏高（> 75%） | — | epsilon 管道窄或 C 大——模型复杂，可能过拟合 |
...
- 支持向量数量比 R^2 更直接地反映 SVR 的复杂度——R^2 告诉你"预测多准"，nSV 告诉你"模型多复杂"。
- 如果 nSV 接近 N_train（如 150/160），模型几乎"记住"了每个样本——过拟合风险极高。
- 如果 nSV 极低（如 5/160），epsilon 管道覆盖了绝大多数样本——模型可能过于简单。
...
| 特征重要性（线性核） | 可输出（`coef_`） | 线性核 SVR 有 `coef_` 属性——相当于带 epsilon-管道的线性回归 |
...
| 好 | 训练 ≫ 验证 | 高 | **过拟合——模型复杂但泛化差，需增大 epsilon 或减小 C** |
| 差 | 训练 ~= 验证（两者都低） | 低 | **欠拟合——模型太简单，需减小 epsilon 或增大 C** |
| 系统性弯曲（如 U 形） | 训练 ~= 验证 | — | **非线性捕捉不足——尝试调整 gamma** |

### 理解重点

- 单独看任何一个指标都可能给出片面判断——三者联合解读才有完整图景。
- nSV 是最快的诊断入口——如果 nSV 在 50~80（30%~50%），通常说明模型复杂度合理。
- 残差图的系统性弯曲是最明确的"核函数/参数不匹配"信号——应优先于 R^2 数值被关注。
...
| MSE / MAE / R^2 数值打印 | **未实现** | 当前流水线侧重图形化诊断 |
| 交叉验证 R^2 均值 | **未实现** | 学习曲线内部使用 CV，但未单独输出均值 |
...
- 支持向量数量是 SVR 独有的诊断指标——比 R^2 更直接地反映模型复杂度。
...
2. 通过动手练习在代码层面观察 C、epsilon、gamma 和支持向量数量的联动关系。
3. 提供扩展阅读的参考文献入口。
...
| 名称 | 类型 | 作用 |
|---|---|---|
| 自检问题 | 诊断 | 确认对 epsilon-管道、RBF 核、支持向量稀疏性、C/epsilon/gamma 角色等核心概念的理解 |
| 动手练习 | 实践 | 修改 C、epsilon、kernel、gamma 并观察 nSV、残差图、学习曲线的变化 |
| 参考文献 | 入口 | 提供 SVR 经典论文和 scikit-learn 官方文档 |

## 1. 自检问题

1. SVR 的 epsilon-不敏感损失函数与 OLS 的平方损失有什么根本区别？为什么管道内的样本不参与预测？
...
回答：C=0.1 时支持向量数量是否显著减少？C=100 时训练 R^2 是否接近 1.0 但验证 R^2 下降？残差图在三个 C 值下有何不同？
...
回答：epsilon=0.01 时支持向量数量是否急剧增加？epsilon=1.0 时是否几乎所有样本都在管道内（nSV 极小）？残差图的散点分布随 epsilon 如何变化？
...
回答：gamma=0.01 时学习曲线的训练/验证间隙是否缩小（更平滑）？gamma=1.0 时训练 R^2 是否极高但验证 R^2 暴跌（严重过拟合）？nSV 随 gamma 如何变化？

### 练习 5：手动加入 R^2 和 MSE 计算

在流水线预测后手动计算并打印 R^2 和 MSE：
...
回答：R^2 是否与学习曲线中的验证得分一致？"R^2 更高"和"nSV 更少"是否同时发生？数值指标与残差图的视觉判断是否吻合？

## 3. 参考文献

| 序号 | 文献 | 说明 |
|---|---|---|
| 1 | Smola, A. J., & Schölkopf, B. (2004). A tutorial on support vector regression. *Statistics and Computing*, 14(3), 199-222. | SVR 经典教程——从 epsilon-不敏感损失到对偶问题的完整推导 |
| 2 | Drucker, H., Burges, C. J., Kaufman, L., Smola, A., & Vapnik, V. (1997). Support vector regression machines. *Advances in Neural Information Processing Systems*, 9. | SVR 原始论文——epsilon-SVR 的提出与算法实现 |
...
1. 修改 C 时忘记 C 是正则化倒数——增大 C = 减弱正则化，与 Lasso 的 alpha 方向相反。
...
- 7 个自检问题覆盖 SVR 的核心概念：$\varepsilon$-管道损失、C 的角色、RBF 核、参数空间、标准化必要性、支持向量数量、PipelineSpec 配置。
- 5 个动手练习从不同角度探索 SVR 行为——调 C、调 $\varepsilon$、切换核函数、调 $\gamma$、加入数值指标。
