...
理解从**右侧开始对齐**，本质上是 **比较两个数组形状时，先看最后的维度，再依次往前看**。

可以这样想：NumPy 把**最右边**的维度当作“最内层”或“最低层”的维度，比较优先级最高。
...
因为广播的规则是：
1. 如果两个数组维度数不同，先在**维度少的形状左边补 1**。
2. 然后从**最后一个维度（最右边）开始**，逐个向前比较两个形状的对应维度，判断是否相等或有一个是 1。
...
**例子 1：** `(3, 4)` 和 `(4,)`
- 右形状 `(4,)` 维度少，先在**左边**补 1，变成 `(1, 4)`。
- 现在两个形状为 `(3, 4)` 和 `(1, 4)`，**从右侧对齐**比较：
  - 右侧第一维：`4` 和 `4` → 相等，取 `4`
  - 然后往左一维：`3` 和 `1` → 满足一个为 1，取 `3`
- 最终广播形状 `(3, 4)`。

如果变成从左开始对齐，那 `3` 和 `1` 先比较，后面的 `4` 和 `4` 再比，虽然结果一样，但不对齐时很容易出错。尤其维度数不同时，“右侧对齐”决定了补 1 的位置。

---

**例子 2：** `(2, 3, 4)` 和 `(3, 4)`
- `(3, 4)` 维度少，左边补 1 变成 `(1, 3, 4)`。
- 右侧对齐比较：
  - 最右边（第2维）：`4` 和 `4` → 取 4
  - 中间（第1维）：`3` 和 `3` → 取 3
  - 最左边（第0维）：`2` 和 `1` → 取 2
...
如果把 `(3, 4)` 补 1 放到**右边**变成 `(3, 4, 1)`，那对齐第2维 `4` 和 `1` 还能兼容（取 4），但第1维 `3` 和 `4` 不相等且都不是 1，就会直接报错。所以**必须左补 1 且从右比较**。

---

#### 一个直觉类比

就像做加法竖式时，我们会**把个位对齐**（最右侧对齐），然后向左逐位相加。广播也是把**最内层的维度对齐**（即形状元组的最右边），再依次向左扩展。

可以说，“从右侧开始对齐”就是为了保证维度间的对应关系是**从最后一维开始，一一匹配**，这样广播出来的数组才有明确的物理意义（不会改变元素的内层结构）。
