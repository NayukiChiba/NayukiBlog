...
image: https://img.yumeko.site/file/blog/cover/1780581893969.webp
status: published
---
...
两个 Loss 都高，两个 Acc 都低，且没有拉开差距——[[NeuralNetwork/Troubleshooting/SlowConvergence|收敛慢]]。如果 Loss 完全不动，则是[[NeuralNetwork/Troubleshooting/LossNotDown|Loss不降]]的问题。模型连训练集都学不好，说明**模型容量不足以表达训练数据中的模式**。
...
```python
# 检查 Resize 是否把图像缩得太小
transform = transforms.Resize((8, 8))  # 太小！28x28 的 MNIST 缩到 8x8 丢失太多信息
transform = transforms.Resize((32, 32))  # 更好的选择
```

## 欠拟合 vs [[NeuralNetwork/Troubleshooting/Overfitting|过拟合]]：快速判断
...
6. 检查数据预处理是否丢失信息
```
![Underfitting.png](https://img.yumeko.site/file/blog/articles/1780581594825.webp)
