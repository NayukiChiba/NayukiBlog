...
|------|:---:|------|:---:|:---:|:---:|------|
| Elman RNN | 1990 | 隐藏状态循环 | 1$\times$ | 差 | 差（<20步） | 简单序列预测 |
| LSTM | 1997 | 三门 + 细胞状态 CEC | ~3.3$\times$ | **优** | **优**（>500步） | 语音识别、机器翻译、文本生成 |
| BiRNN | 1997 | 正向+反向双通道 | ~2$\times$ | 同基础单元 | 同基础单元 | 序列标注、情感分类 |
| GRU | 2014 | 两门 + 统一状态 | ~2.5$\times$ | 优 | 优（200-500步） | 快速实验、资源受限场景 |
| Seq2Seq | 2014 | 编码器-解码器 | 2$\times$ 基础单元 | 同基础单元 | 同基础单元 | 机器翻译、文本摘要 |
...
**参数量说明**：以 Elman RNN 单层参数量为基准（$\approx$ $h^2 + h \cdot i$）。LSTM 约为 4 倍（四套门参数），GRU 约为 3 倍（三套门参数但无独立 $C_t$）。
