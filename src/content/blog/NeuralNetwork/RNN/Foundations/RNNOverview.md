...
对于序列数据，这意味着：
- 截断 → 丢失长序列尾部的重要信息
- 填充 → 浪费计算在无意义的零值上，且引入了人为的噪声模式

### 2.2 局限二：无状态记忆

MLP 的每一次前向传播是**完全独立的**。处理第 t 个词时，输入中不包含第 t-1 个词的信息。

```
MLP 处理句子 "I love you":
  "I"     → MLP → 输出     （独立计算）
  "love"  → MLP → 输出     （独立计算，不包含前面"I"）
  "you"   → MLP → 输出     （独立计算，不包含前面"I love"）
```
...
**应用**：
- 图像描述生成（Image Captioning）：输入=图像特征，输出=描述文本
- 音乐生成：输入=风格/音调条件，输出=音符序列

### 4.3 Many-to-One（多对一）

输入一个序列，输出一个向量。这是**序列分类**的标准模式。
...
**应用**：
- 情感分析：输入=评论词序列，输出=正面/负面
- 文本主题分类：输入=文章，输出=类别
- EmotionClassification 项目使用此模式

### 4.4 Many-to-Many（同步）

输入一个序列，输出一个等长的序列。每个输入位置对应一个输出标签。
...
**应用**：
- 词性标注（POS Tagging）：每个词 → 词性标签
- 命名实体识别（NER）：每个词 → B/I/O 标签
- 视频帧分类：每帧 → 动作类别

### 4.5 Many-to-Many（异步）

输入一个序列，输出一个不同长度的序列。这是**Seq2Seq**模式。

```
x_1 → x_2 → ... → x_T → [编码器] → 上下文 → [解码器] → y_1 → y_2 → ... → y_S
```

**应用**：
- 机器翻译：中文句子 → 英文句子（长度不同）
- 文本摘要：长文 → 短文
- Char-RNN 的自回归生成也是此模式（只是输入和输出是同一个序列的前后关系）
...
| 感受野/记忆范围 | 全连接（看全部输入） | 局部→全局（层次化） | **逐步累积**（隐藏状态传递） |
| 并行性 | 高 | 高 | **低**（时序依赖） |
