...
- $i_t$：决定在哪些位置写入新信息（Sigmoid → 0~1 的开关）
- $\tilde{C}_t$：生成候选的新内容（Tanh → -1~1 的信息）
...
| 项目                    | input_size | hidden_size | num_layers | bidirectional | 每方向单层参数 |
| --------------------- | :--------: | :---------: | :--------: | :-----------: | ------: |
| Char-RNN              |    256     |     256     |     2      |       否       | 526,336 |
| EmotionClassification |    300     |     256     |     2      |       是       | 594,944 |
...
```python
# 正向最后一步
forwardLast = hn[-2, :, :]    # 倒数第2层，正向

# 反向最后一步
backwardLast = hn[-1, :, :]   # 倒数第1层，反向

# 拼接
finalHidden = torch.cat([forwardLast, backwardLast], dim=-1)  # 512维
```
