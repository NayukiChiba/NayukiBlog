...
| 是否有输出门 | [OK]（控制对外暴露多少） | [NG]（$h_t$ 直接输出） |
...
| 组件 | LSTM | GRU |
|------|------:|------:|
| weight_ih | 4$\times$256$\times$256 = 262,144 | 3$\times$256$\times$256 = 196,608 |
| weight_hh | 4$\times$256$\times$256 = 262,144 | 3$\times$256$\times$256 = 196,608 |
| bias | 4$\times$256$\times$2 = 2,048 | 3$\times$256$\times$2 = 1,536 |
...
| 公式编号 | 数学公式 | 对应代码行 |
|:---:|------|------|
| 1 | $r_t = \sigma(W_{ir} \cdot x_t + b_{ir} + W_{hr} \cdot h_{t-1} + b_{hr})$ | `self.W_ir(x)` 对应 $W_{ir} \cdot x_t + b_{ir}$，`self.W_hr(h)` 对应 $W_{hr} \cdot h_{t-1} + b_{hr}$，两者相加后过 `torch.sigmoid` |
| 2 | $z_t = \sigma(W_{iz} \cdot x_t + b_{iz} + W_{hz} \cdot h_{t-1} + b_{hz})$ | 结构与 1 完全相同，用独立的参数 `W_iz` / `W_hz` |
| 3 | $\tilde{h}_t = \tanh(W_{i\tilde{h}} \cdot x_t + b_{i\tilde{h}} + r_t \odot (W_{h\tilde{h}} \cdot h_{t-1} + b_{h\tilde{h}}))$ | `self.W_in(x)` 处理 $x_t$，`r_t * self.W_hn(h)` 实现 $r_t \odot (W_{h\tilde{h}} \cdot h_{t-1} + b_{h\tilde{h}})$，两者相加后过 `torch.tanh` |
| 4 | $h_t = (1 - z_t) \odot h_{t-1} + z_t \odot \tilde{h}_t$ | `(1 - z_t) * h + z_t * n_t`，`*` 在 PyTorch 中是逐元素乘法，对应 $\odot$ |
...
output, hFinal = gru(x)
# output:  (64, 128, 512)  每步的输出（双向拼接: 256x2）
# hFinal:  (4, 64, 256)    最后一层的双向最终隐藏状态（2层x2方向=4）
```
