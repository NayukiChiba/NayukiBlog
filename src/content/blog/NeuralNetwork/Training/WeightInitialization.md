...
- **初始化太大** $\rightarrow$ 激活值进入饱和区（Tanh/Sigmoid 两端导数趋近于 0）$\rightarrow$ 梯度消失 $\rightarrow$ 学不动
- **初始化太小** $\rightarrow$ 信号在网络传播中越来越微弱 $\rightarrow$ 深层几乎收不到信号 $\rightarrow$ 同样学不动
- **初始化全为 0** $\rightarrow$ 所有神经元做完全相同的计算 $\rightarrow$ 对称性问题，等价于只有 1 个神经元
...
假设一层有 $n_{\text{in}}$ 个输入神经元和 $n_{\text{out}}$ 个输出神经元。为了让输出的方差 $\approx$ 输入的方差：
