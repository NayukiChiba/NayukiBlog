...
数据增强应该模拟真实场景中可能出现的变换，它是防止[[NeuralNetwork/Troubleshooting/Overfitting|过拟合]]最有效的正则化手段之一。
...
1. **只在训练集上做增强**，验证集和测试集只用基础[[NeuralNetwork/CNN/DataPreprocessing|数据预处理]]（ToTensor + Normalize）
