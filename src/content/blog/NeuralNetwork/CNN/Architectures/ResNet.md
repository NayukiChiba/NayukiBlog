...
[//]: # (TODO: 退化问题对比图 — 56层 vs 20层训练/测试误差曲线)
...
![TODO: 残差块结构图，输入x分两路——主干Weight Layer→ReLU→Weight Layer，捷径identity直接连接，汇合经ReLU输出F(x)+x]
...
![TODO: ResNet-34完整架构图，初始7x7卷积→4个stage包含[3,4,6,3]个残差块→GAP→FC]
...
Bottleneck 将参数量降低了约 17 倍！这使得 152 层的 ResNet（参数量 60M）反而比 [[NeuralNetwork/CNN/Architectures/VGG|VGG]]-16（138M）更轻量。
...
实验发现，ResNet 的损失曲面比 VGG 等普通网络更平滑，优化器更容易找到好的最小值。残差连接的直观效果就是让损失曲面"变好"。与 [[NeuralNetwork/CNN/Architectures/GoogLeNet|GoogLeNet]] 的 Inception 多分支不同，ResNet 通过简单的加法跳跃实现了更深层的训练。
...
ResNet 的残差连接思想已成为深度学习架构设计的默认范式，从 [[NeuralNetwork/CNN/Architectures/GoogLeNet|GoogLeNet]] 的并行多分支到 ResNet 的残差跳跃，每一次架构演进都在解决不同的核心矛盾。
