...
```mermaid
graph TD
    Req["客户端请求"] --> API["app/api<br>(路由层)"]
    
    API -.-> Core["app/core<br>(配置/安全)"]
    API -- 校验/格式化 --> Schemas["app/schemas<br>(数据传输模型)"]
    
    API -- 调用业务逻辑 --> Services["app/services<br>(业务逻辑层)"]
    Services -- 数据库操作 --> CRUD["app/crud<br>(数据库操作)"]
    CRUD -- ORM映射 --> Models["app/models<br>(数据库表模型)"]
    Models -.-> DB[("SQLite")]
